...
public class Reporting extends TestListenerAdapter {
	...
	public void onStart(ITestContext testContext) {
		...
		htmlReporter.config().setDocumentTitle("INET Banking"); // Tile of report
	...
	public void onTestSuccess(ITestResult tr) {
		...
		logger.log(Status.INFO, tr.getParameters()[0].toString());
	...
	public void onTestFailure(ITestResult tr) {
		...
		if (f.exists()) {
			try {
				...
				logger.log(Status.INFO, tr.getMethod().getMethodName());
				logger.log(Status.INFO, tr.getParameters()[0].toString());
				logger.log(Status.INFO, tr.getMethod().getMethodName() + tr.getThrowable().getMessage());
